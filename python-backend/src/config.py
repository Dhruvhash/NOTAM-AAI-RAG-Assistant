...
CHAT_MODEL = "llama-3.1-8b-instant"   # cheaper/higher free-tier limit than 70b
...
def resolve_icao_codes(query: str) -> list[str]:
    ...
    tokens = [t for t in re.findall(r"[A-Z]{3,4}", query_upper)]
    matches = set()

    # Common English words to ignore when scanning 4-letter tokens
    STOP_WORDS = {"WITH", "FROM", "THIS", "THAT", "HAVE", "ALSO", "SOME", "WHAT", "WHEN", "INFO", "DATA", "TEST", "DUBA", "HEAT", "HROW", "SING", "APOR"}
    ...
    for token in tokens:
        ...
        # 3. Direct 4-letter ICAO typed by user (e.g. EGLL, KJFK, OMDB, VECC, VOHS, WSSS)
        elif len(token) == 4 and token not in STOP_WORDS:
            matches.add(token)

    # 4. Check airport names (e.g. "heathrow" -> EGLL, "dubai" -> OMDB, "singapore" -> WSSS, "kolkata" -> VECC)
